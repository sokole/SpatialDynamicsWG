####################################################################
# -- Biodiversity (invert) metrics -- pop comm group -- Stream Resiliency RCN
# -- -- updated 17 April 2020
# -- -- Eric Sokol, Darin Kopp
...
results_all_list <- calculate_biodiversity_metrics(
  ...
  taxon_id_vector = biodata_cleaned$acceptedname)
...
for(i_region in region_list){
  ...
  try({
    ...
    results_all_list <- calculate_biodiversity_metrics(
      ...
      taxon_id_vector = data_chunk$acceptedname)
